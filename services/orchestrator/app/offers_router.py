...
from services.matching_engine.app.routing import get_road_eta
...
async def fan_out_offers(
    ...
    for offer in offers:
        ...
        views.append(
            owner_view(offer, reputation.to_public_dict() if reputation else None)
        )
...
async def get_offer(
    ...
    if role == "owner":
        reputation = await get_reputation(db, offer.carrier_company_id)
        return owner_view(offer, reputation.to_public_dict() if reputation else None)
    if role == "driver":
...
async def confirm_as_owner(
    ...
    return {
        "bound": bound,
        "offer": owner_view(offer, reputation.to_public_dict() if reputation else None),
    }
